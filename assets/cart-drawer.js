class CartDrawer extends HTMLElement {
  ...
  setHeaderCartIconAccessibility() {
    ...
    cartLink.addEventListener('click', (event) => {
      ...
      // On cart page, don't intercept - let the link work normally (reload)
  ...
  open(triggeredBy) {
    // Prevent drawer from opening when on cart page
    const isCartPage = window.location.pathname === '/cart' || window.location.pathname.includes('/cart');
    if (isCartPage) {
      return;
    }
  ...
  renderContents(parsedState, options = {}) {
    // Bug Fix: Don't open drawer when on cart page (delete actions shouldn't open drawer)
    const isCartPage = window.location.pathname === '/cart' || window.location.pathname.includes('/cart');
    const shouldOpen = options.openDrawer !== false && !isCartPage;
    ...
    // Only open drawer if not on cart page and openDrawer is not explicitly false
    if (shouldOpen) {
      setTimeout(() => {
        this.querySelector('#CartDrawer-Overlay').addEventListener('click', this.close.bind(this));
        this.open();
